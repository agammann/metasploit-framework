...
Docker Desktop, adds a command-line toolbox, and optionally opens Rust CVE
Sniffer on Windows. The console in that image identifies itself as `6.5.5-dev`.
...
To open Rust CVE Sniffer with Metasploit, double-click
**`personal/Install-Rust-Cve-Sniffer.cmd`** once before the first launch. It is
optional; the toolkit runs without it.
...
can take several minutes. Rust CVE Sniffer opens with the toolkit after its
...
data. Close the separate Rust CVE Sniffer window yourself when finished.

### What the buttons do

| Button | Result |
| --- | --- |
| `Launch-Metasploit.cmd` | Checks Docker and the host port, builds or reuses the pinned image, starts the database, opens the enabled Toolbox and installed Rust CVE Sniffer, and opens one Metasploit console. A second click while that console runs opens missing windows without creating another console. |
...
| `Stop-Metasploit.cmd` | Stops the Metasploit console, Toolbox, and database containers. Docker volumes and local files remain. Close Rust CVE Sniffer separately. |
...
Sniffer is the optional Windows desktop app described above.
...
| TShark | Reads and filters existing packet-capture files under `/workspace`. Live capture from the Windows host is outside this container. |
...
in that image, not a separate Metasploit installation. Rust CVE Sniffer runs on
Windows outside Docker.
...
| Window preferences and portable Rust scanner | Ignored `personal/companions.local.json` and `personal/tools/`. |
...
still needs the upstream package and source servers. Rust CVE Sniffer has its
own pinned release and update schedule.

### Choose which windows open

Copy `companions.example.json` to `companions.local.json` inside `personal/`.
Set `OpenToolbox` or `OpenRustCveSniffer` to `false` to skip that automatic
window. Leave `RustCveSnifferPath` blank for the standard portable install, or
set it to a custom `Start Scanner.bat` path. If the scanner is not installed,
the Metasploit launch continues without it.
...
your ignored `.env`, workspace files, local window settings, and Docker
...
| Rust CVE Sniffer does not open | Run `Install-Rust-Cve-Sniffer.cmd` once, then relaunch. For a custom location, set `RustCveSnifferPath` in `companions.local.json`. |
